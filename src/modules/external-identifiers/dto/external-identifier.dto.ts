...
import { z } from 'zod';
import { dateStringSchema, requiredString, verificationStatusSchema } from '../../../common/dto/schemas';

const externalIdentifierFields = z.object({
...
const validateDates = (value: { validFrom?: Date | null; validTo?: Date | null }, ctx: z.RefinementCtx) => {
    if (value.validFrom && value.validTo && value.validTo < value.validFrom) {
      ctx.addIssue({ code: 'custom', path: ['validTo'], message: 'validTo must not be earlier than validFrom' });
    }
  };
...
export class CreateExternalIdentifierDto {
  ...
  @ApiPropertyOptional({enum:VerificationStatus,default:VerificationStatus.UNVERIFIED}) verificationStatus:VerificationStatus=VerificationStatus.UNVERIFIED;
...
export class UpdateExternalIdentifierDto {
  static readonly schema = externalIdentifierFields.partial().superRefine(validateDates);
