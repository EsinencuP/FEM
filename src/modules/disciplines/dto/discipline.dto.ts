...
import { archivedSchema, recordStatusSchema, requiredString, sortOrderSchema } from '../../../common/dto/schemas';
...
export class DisciplineListQueryDto {
  static readonly schema = paginationSchema.extend({
  ...
  @ApiPropertyOptional({ enum: RecordStatus }) status?: RecordStatus;
  @ApiPropertyOptional({ enum: ['true', 'false', 'all'], default: 'false' }) archived: 'true' | 'false' | 'all' = 'false';
  @ApiPropertyOptional({ enum: ['name', 'code', 'createdAt'], default: 'name' }) sortBy: 'name' | 'code' | 'createdAt' = 'name';
