...
Для текущего DB-first demo канонический scope находится в корневом
`FEM_MVP_ACCELERATED_PLAN.md`. Корневой `DOCS_REPLACEMENT_MAP.md` определяет,
какие документы заменяются или обновляются. Одноимённые копии в `docs/`
создавать нельзя.
...
После намеренного удаления или перемещения файлов используется подтверждённая
текущей версией команда:

```powershell
graphify update . --force
```

Она разрешает Graphify уменьшить граф и удалить stale nodes. Для обычных
изменений `--force` не нужен.
...
2. Прочитать активный scope и карту замены документов.
3. Убедиться, что граф актуален.
4. Прочитать `graphify-out/GRAPH_REPORT.md`.
5. Выполнить `query` по задаче.
6. Выполнить `path` для критических зависимостей.
7. Открыть только релевантные исходные файлы и проверить выводы графа.
8. Внести изменения.
9. Запустить lint, typecheck, tests и build.
10. Обновить граф.
11. Проверить новые god nodes, cycles и неожиданные связи.
